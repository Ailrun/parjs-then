 ...
 * @param next
 */
export function butThen<T>(next: ImplicitParjser<T>): ParjsCombinator<unknown, T> {
 ...
 * @param next
 */
export function followedBy<T>(next: ImplicitParjser<unknown>): ParjsCombinator<T, T> {
    return composeCombinator(
        andThen(next),
        map(arr => arr[0])
    );
...
class AndThen<T, Rest extends CombinatorInput<unknown>[]> extends Combinated<
    T,
    [
        T,
        ...{
...
export function andThen<T, Parsers extends ImplicitParjser<unknown>[]>(
    ...parsers: Parsers
    ...
    return source => {
        return new AndThen<T, typeof resolvedParsers>(wrapImplicit(source), resolvedParsers);
    };
}
