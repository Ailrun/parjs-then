...
export const comment = token(anyCharOf(";#"))
    .pipe(butThen(anyCharOf(valueCharacters).pipe(many(), stringify())))
    .expects("comment");
...
export const definitionLine = token(identifier)
    .pipe(
        followedBy(token(string("="))),
        andThen(value, comment.pipe(maybe())),
        followedBy(newline().pipe(or(eof()))),
        map(([name, val]) => new Property(name.toLowerCase(), val))
    )
    .expects("definition");

const emptyLine = comment
    .pipe(maybe(), butThen(maybeSpacesOrTabs))
    .pipe(butThen(newline()), butThen(result(new EmptyLine())))
    .expects("empty line");
...
export const sectionHeader = token(
    ...
    .pipe(followedBy(comment.pipe(maybe())))
    .expects("section header");

export const section = sectionHeader
    .pipe(
        followedBy(newline()),
        andThen(definitionList),
