    ...
    describe("regexp", () => {
        describe("simple regexp", () => {
            ...
            it("succeds using implicit", () => {
                expect(string("abc").pipe(andThen(/abc/)).parse("abcabc")).toBeSuccessful([
                    "abc",
                    ["abc"]
                ]);
            });
            ...
            it("match starts in the proper location", () => {
                const p = string("abc");
                expect(p.pipe(andThen(parser)).parse("abcabc")).toBeSuccessful(["abc", ["abc"]]);
            });
            it("match ends in the proper location", () => {
                const p1 = string("abc");
                const p2 = regexp(/.{3}/);
                const p3 = string("eeee");
                const r = p1.pipe(andThen(p2, p3));
        ...
        describe("multi-match regexp", () => {
            ...
            const parser2 = parser.pipe(andThen("de"));
            it("chains correctly", () => {
