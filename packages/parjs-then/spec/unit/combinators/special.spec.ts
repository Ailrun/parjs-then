...
import { andThen, backtrack, each, map, replaceState } from "@lib/combinators";

describe("special combinators", () => {
    describe("backtrack", () => {
        const parser = string("hi").pipe(andThen(eof()), backtrack());
        ...
        it("succeeds if inner succeeds, non-zero match", () => {
            ...
            const redundantParser = parseHi.pipe(backtrack(), andThen("his"));
