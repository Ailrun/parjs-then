...
describe("sequential combinators", () => {
    describe("followedBy", () => {
        const parser = prs.pipe(followedBy(prs2));
        it("succeeds", () => {
    ...
    describe("butThen", () => {
        const parser = prs.pipe(butThen(prs2));
        it("succeeds", () => {
    ...
    describe("andThen", () => {
        describe("1 arg", () => {
            const parser = prs.pipe(andThen(prs2));
            it("succeeds", () => {
        ...
        describe("1 arg, then zero consume", () => {
            const parser = prs.pipe(andThen(prs2), followedBy(eof()));
            it("succeeds", () => {
                expect(parser.parse(goodInput)).toBeSuccessful(["ab", "cd"]);
            });
    ...
    describe("many combinators", () => {
        describe("regular many", () => {
            ...
            it("chains to EOF correctly", () => {
                const endEof = parser.pipe(followedBy(eof()));
    ...
    describe("manySepBy combinator", () => {
        ...
        it("works with max iterations", () => {
            ...
            const parser3 = parser2.pipe(followedBy(string(", ab")));
        ...
        it("many that fails hard on 2nd iteration", () => {
            const manyParser = string("a").pipe(andThen("b"), stringify(), manySepBy(", "));
        ...
        it("chains into terminating separator", () => {
            const parser2 = parser.pipe(followedBy(", "));
    ...
    describe("manyTill combinator", () => {
        ...
        it("succeeds matching 1 then till, chains", () => {
            const parser2 = parser.pipe(followedBy(prs));
