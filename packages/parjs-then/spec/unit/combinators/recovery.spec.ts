...
describe("maybe combinator", () => {
    ...
    it("causes progress on success", () => {
        const p = string("abc").pipe(maybe(), butThen("123"));
...
describe("or val combinator", () => {
    const parser = string("a").pipe(andThen("b"), stringify(), maybe("c"));

    const p2: Parjser<[0 | "a", "b"]> = string("a").pipe(maybe(0), andThen(string("b")));
...
describe("not combinator", () => {
    const parser = string("a").pipe(andThen("b"), stringify(), not());
    ...
    it("succeeds on hard fail if we take care of the rest", () => {
        const parser2 = parser.pipe(andThen(rest()));
